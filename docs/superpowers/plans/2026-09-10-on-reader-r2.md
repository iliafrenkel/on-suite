...
Every count in this plan is a single `count(*)` with a `NOT EXISTS` correlated
subquery, which is exact and — with `reader_item_state`'s `(user_id, item_id)`
primary key — indexed.

This is worth stating because an earlier draft of the design spec said the
count was `count(items) - count(read-states)`, "two indexed counts". That is
wrong: subtraction is only correct if every read-state row corresponds to an
item inside the counted range, and `reader_subs.added_at` means it does not — a
user can hold a read-state row for an item published before they subscribed.
Subtracting it undercounts, hiding a genuinely unread article rather than
failing visibly.

Section 4 of the spec has been amended to match. If you find any query in this
plan doing the subtraction, it is a bug in the plan, not a shortcut to take.
